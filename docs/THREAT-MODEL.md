...
| CPI updater submits an extreme or rapid value | Absolute bounds, step limit, and minimum interval; DAO can revoke the role or use `mockCPI` | A compromised updater can still move the rate within allowed limits over time; use a governed, monitored updater |
