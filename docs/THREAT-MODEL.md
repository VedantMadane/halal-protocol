...
| DAO proposal drains reserves, expands adapter work, or grants minting | Governor snapshot voting plus timelock delay; PSM reserve floor and `CPIReportAdapter.MAX_SIGNERS` bound ordinary reserve and report-verification work | Governance capture remains a protocol-level risk; use multisig-controlled beneficiaries and monitor proposals |
| CPI adapter ownership handoff becomes stuck or combines custody with quorum authority | Active and pending adapter owners are rejected from the signer set; ownership acceptance also rejects signer owners | Governance must still review signer rotation and complete the two-step ownership handoff |
...
| A dependency address is an EOA or inert account | DAO, PSM, vesting, and CPI adapter constructors reject non-contract dependencies; deployment verifier also checks bytecode | Review the exact deployed bytecode and wiring before granting roles |
...
- The current CPI paths are bounded report submissions, not a live Chainlink Functions consumer.
  ...
  cadence. The PSM rejects deposits while the watermark is empty or stale; the frontend repeats
  that gate to give users an explanation before signing.
